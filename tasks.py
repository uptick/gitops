...
def deploy(ctx):
    ...
    run((
        ...
        f" --set environment.CLUSTER_NAME={cluster_details['name']}"  # ---- TODO: Automatically fetch from user's current settings
        f" --set secrets.ACCOUNT_ID={b64encode(str(ACCOUNT_ID).encode()).decode()}"  # ---- TODO: Automatically fetch from user's current settings
